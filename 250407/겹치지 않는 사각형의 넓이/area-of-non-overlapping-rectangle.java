...
public class Main {
    ...
    private static int[][] area = new int[LENGTH+1][LENGTH+1];
    public static void main(String[] args) throws IOException{
        ...
        for (int i=0; i<3; i++){
            ...
            color(x[i][0],x[i][1],y[i][0],y[i][1],i+1);
        }
        
        int cnt = 0;
        for (int i=0;i<=LENGTH;i++){
            for (int j=0;j<=LENGTH;j++){
                if (area[i][j]==1||area[i][j]==2) cnt++;
            }
        }
    ...
    // 범위만큼 직사각형 c로 칠하기
    private static void color(int x1,int x2,int y1,int y2,int value){
        for (int r = x1; r<x2; r++){
            for (int c = y1;c<y2;c++){
                area[r][c] = value;
            }
